<script>
    let activeTab = 'YOUTBERS';

    const tabs = ['YOUTBERS', 'SITE:S', 'QUOTES', 'SONGS'];
...
<div class="wrapper">
    ...
    <div class="content">
        {#each content[activeTab] as item}
            <a class="content-item" href={item.url} target="_blank" rel="noopener noreferrer">
                {item.text}
            </a>
        {/each}
    </div>
